...
type WorkspaceAlgorithm interface {
...
func (t *WorkspaceStats) GetRouterPickupJobs(destType string, noOfWorkers int, routerTimeOut time.Duration, jobQueryBatchSize int, timeGained float64) map[string]int {
	workspacesWithJobs := getWorkspacesWithPendingJobs(destType, t.routerTenantLatencyStat[destType])
	boostedRouterTimeOut := multitenant.getBoostedRouterTimeOut(routerTimeOut, timeGained, noOfWorkers)
	...
	minLatency, maxLatency := multitenant.getMinMaxWorkspaceLatency(workspacesWithJobs, t.routerTenantLatencyStat[destType])
			...
			if ok {
				...
				if t.routerTenantLatencyStat[destType][workspaceKey].Value() != 0 {
					...
					if tmpPickCount < 1 {
						...
						multitenant.pkgLogger.Debugf("[DRAIN DEBUG] %v  checking for high latency/low in rate workspace %v latency value %v in rate %v", destType, workspaceKey, t.routerTenantLatencyStat[destType][workspaceKey].Value(), destTypeCount.Value())
				...
				multitenant.pkgLogger.Debugf("Time Calculated : %v , Remaining Time : %v , Workspace : %v ,runningJobCount : %v , moving_average_latency : %v, routerInRate : %v ,DestType : %v,InRateLoop ", timeRequired, runningTimeCounter, workspaceKey, runningJobCount, t.routerTenantLatencyStat[destType][workspaceKey].Value(), destTypeCount.Value(), destType)
			}
		}
	}
