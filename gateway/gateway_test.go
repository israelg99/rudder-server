...
var _ = Describe("Gateway Enterprise", func() {
	...
	BeforeEach(func() {
		...
		c.mockSuppressUser.EXPECT().IsSuppressedUser(gomock.Any(), NormalUserID, SourceIDEnabled).Return(false).AnyTimes()
		c.mockSuppressUser.EXPECT().IsSuppressedUser(gomock.Any(), SuppressedUserID, SourceIDEnabled).Return(true).AnyTimes()
