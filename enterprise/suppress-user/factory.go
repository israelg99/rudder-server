...
func (m *Factory) Setup(backendConfig backendconfig.BackendConfig) (types.UserSuppression, error) {
	...
	// TODO: we should respect the startup context
	ctx := context.TODO()
	...
	if config.GetBool("BackendConfig.Regulations.useBadgerDB", true) {
		...
		// TODO: implement seeder source, to retrieve the initial state from the backup
		...
		repository, err = NewBadgerRepository(
			...
			WithMaxSeedWait(10*time.Second))
		if err != nil {
	...
	syncer := NewSyncer(
		...
		WithPageSize(config.GetInt("BackendConfig.Regulations.pageSize", 50)),
